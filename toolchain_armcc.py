...
#nordic specific stuffz
    ...
    def scatterGen(self, target):
        ...
        altRomObjs = []
        ramplace = {}
        for f in target.allFiles():
            if f.romRegion != target.romRegions[0]:
                altRomObjs.append(f.name.split(".")[0] + ".o")

            ramplace[f.name.split(".")[0] + ".o"] = (f.ramRegion, f.ziRegion)
        ...
        for (i, region) in enumerate(target.romRegions):
            ...
            if region.init:
                for (i, region) in enumerate(target.ramRegions):
                    ...
                    for obj in ramplace.keys():
                        if ramplace[obj][0] != ramplace[obj][1] or (i is 1 and ramplace[obj][0] == region):
                            placestr = ""
                            if ramplace[obj][0] == region:
                                placestr = " +RW"
                            if ramplace[obj][1] == region:
                                placestr += " +ZI"
                            out += "\t\t" + obj + "(" + placestr + " )\n"
                    out += "\t\t.ANY (+RW +ZI)\n"
                    out += "\t}\n"
